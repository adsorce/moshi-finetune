#!/bin/bash
# Recording and Processing Script for STT Training Dataset
# This script provides an interactive workflow to record, process, and transcribe audio samples
...
check_dependencies() {
    ...
    if ! command -v arecord &> /dev/null; then
        echo -e "${RED}Error: arecord is not installed${NC}"
...
record_sample() {
    ...
    echo -e "${RED}RECORDING... Press Ctrl+C to stop${NC}"
    echo "(Recording will auto-stop after ${RECORD_DURATION} seconds)"

    # Set up trap to catch Ctrl+C
    trap 'echo "" > /dev/null' INT

    # Record audio with max duration
    arecord -f S16_LE -r 48000 -c 2 -d ${RECORD_DURATION} "${raw_file}" 2>/dev/null || true
